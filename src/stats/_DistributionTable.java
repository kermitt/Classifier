...
public class _DistributionTable {
	...
	public double getCriticalNumber( int numerator, int denominator, String type) {
		...
		if ( type.equals(_Constants.P_FIVE_PERCENT)) {
			try {
				double[] row = table_fivepercent.get( n - 1);
				criticalNumber = row[d - 1];
			} catch( java.lang.IndexOutOfBoundsException ioobe) {
				double[] row = table_fivepercent.get( table_fivepercent.size() - 1);
				criticalNumber = row[row.length - 1];
			}
		} else if ( type.equals(_Constants.P_ONE_PERCENT)) {
