...
public class TimeSeriesClustering_Step3 extends PSVReader {
	public Map<String, List<String>> time_chunks = new HashMap<>();
	private double[] X_pov, Y_pov, Z_pov;

	public void doClustering() {
		String filepath = Library.DATA_PATH + Library.CLUSTER_FILE;
	...
	public void setup() {
		...
		// IMPORTANT! Create a hyperplane by cutting through the hyper-space
		// along these hyperlocations. I.e., Down-dimensionally cast a shadow
		// into XYZ space from most_drug_label, group and gender POV
		X_pov = Library.getRiv(Library.most_drug_label_name.split(Library.PIPE)[2]);
		Y_pov = Library.getRiv(Library.most_drug_group_description.split(Library.PIPE)[2]);
		// Z_pov =
		// Library.getRiv(Library.most_gender_code.split(Library.PIPE)[2]);
		X_pov = Library.getRandomRiv();
		Y_pov = Library.getRandomRiv();
		Z_pov = Library.getBlankRiv();
	}

	public static void main(String... strings) {
		long t1 = System.currentTimeMillis();
		TimeSeriesClustering_Step3 tsc = new TimeSeriesClustering_Step3();
		tsc.setup();
